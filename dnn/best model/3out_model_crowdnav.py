...
class CSVLoader():
    def __init__(self,csv_file_path=''):
        ...
        self.dynamic_obstacle_occupation=np.hstack(data[:,28:29]).astype('float32')
        
        self.output1=np.hstack([data[:,2:3]])
        self.output2=np.hstack([data[:,3:4]])
        self.output3=np.hstack([data[:,4:5]])
        self.output4=np.hstack([data[:,29:30]])
        self.output5=np.hstack([data[:,30:31]])
        ...
        self.output4=self.output4.astype('float32')
        self.output5=self.output5.astype('float32')
...
class ModelTrain():
    ...
    def TrainModel(args):
        ...
        X=inputs[0]
        #Y=np.array([data_loader.output1,data_loader.output2,data_loader.output3])
        # model architectrue
        input=tf.keras.layers.Input(shape=(inputs[1]))   
       # drop_out = tf.keras.layers.Dropout(0.5)     
        dense1=tf.keras.layers.Dense(units=8,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(input)
        #dense2=tf.keras.layers.Dense(units=8,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense1)
        out2=tf.keras.layers.Dense(units=1,activation=tf.nn.sigmoid)(dense1)  
        
        #merge1=tf.keras.layers.concatenate([input,out2])
        #dense3=tf.keras.layers.Dense(units=256,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(merge1)
        #dense5=tf.keras.layers.Dense(units=256,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense3)
        out1=tf.keras.layers.Dense(units=1,activation=tf.nn.sigmoid)(dense1)
        
        merge2=tf.keras.layers.concatenate([input,out1,out2])
        dense4=tf.keras.layers.Dense(units=64,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(merge2)
        #dense5=tf.keras.layers.Dense(units=16,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense4)
        ...
        out3=tf.keras.layers.Dense(units=1,activation=tf.nn.sigmoid)(dense4)
        ...
        for train, test in kfold.split(X):
           ...
           if args.lr_decay=='True':
               ...
               history=model.fit(X[train],[data_loader.output1[train],data_loader.output2[train],data_loader.output3[train]],epochs=num_epochs,batch_size=batch_size,callbacks=callbacks_list,verbose=args.verbose)
               scores=model.evaluate(X[test], [data_loader.output1[test],data_loader.output2[test],data_loader.output3[test]],verbose=args.verbose)
           else:
               history=model.fit(X[train],[data_loader.output1[train],data_loader.output2[train],data_loader.output3[train]],epochs=num_epochs,batch_size=batch_size,verbose=args.verbose)
               scores=model.evaluate(X[test], [data_loader.output1[test],data_loader.output2[test],data_loader.output3[test]],verbose=args.verbose)
       ...
       #     plt.plot(scores.history['loss'],label='val loss')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        dest="train_csv",
        default=f"/home/parallels/dnn/new/crowdnav0220.csv",
    ...
    parser.add_argument(
        ...
        action="store",
        dest="predic_csv",
        default=f"/home/parallels/dnn/new/crowdnav0220_predict.csv",

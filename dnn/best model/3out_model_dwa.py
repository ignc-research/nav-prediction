...
class ModelTrain():
    ...
    def TrainModel(args):
       ...
       # drop_out = tf.keras.layers.Dropout(0.5)     
        ...
        #dense2=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense1)
        #dense3=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense2)
        #dense4=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense3)
        #dense5=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense4)
        out2=tf.keras.layers.Dense(units=1,activation=tf.nn.sigmoid)(dense1)  
        
        #merge1=tf.keras.layers.concatenate([input,out2])
        #dense3=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(merge1)
        #dense7=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense3)
        #dense8=tf.keras.layers.Dense(units=512,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense7)
        out1=tf.keras.layers.Dense(units=1,activation=tf.nn.sigmoid)(dense1)
        
        #merge2=tf.keras.layers.concatenate([input,out1,out2])
        #dense4=tf.keras.layers.Dense(units=8,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(merge2)
        #dense5=tf.keras.layers.Dense(units=8,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense4)
        #dense6=tf.keras.layers.Dense(units=8,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(dense5)
        
             
        #merge=tf.keras.layers.concatenate([input,out1,out2])
        #dense2=tf.keras.layers.Dense(units=1024,kernel_regularizer=tf.keras.regularizers.l2(0.01),activation=tf.nn.relu)(merge)
        #out3=tf.keras.layers.Dense(units=1,activation=tf.nn.sigmoid)(dense1)
        
        model=tf.keras.Model(input,[out1,out2])
        ...
        ########trainning model
        model.compile(optimizer=optimizer,loss=[loss,loss], metrics=['mse'])
        if args.lr_decay=='True':
            print("learning decay")
            lrate = tf.keras.callbacks.LearningRateScheduler(ModelTrain.step_decay)
            history=model.fit(X, [data_loader.output1,data_loader.output2],epochs=num_epochs,batch_size=batch_size,callbacks=[lrate],verbose=args.verbose,validation_split=0.33,)
        else:
            history=model.fit(X, [data_loader.output1,data_loader.output2],epochs=num_epochs,batch_size=batch_size,verbose=args.verbose,validation_split=0.33,)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        dest="train_csv",
        default=f"/home/parallels/dnn/new/dwa0220.csv",
    ...
    parser.add_argument(
        ...
        action="store",
        dest="predic_csv",
        default=f"/home/parallels/dnn/new/dwa0220_predict.csv",
